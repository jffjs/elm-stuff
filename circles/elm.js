...
Elm.CirclesModel.make = function (_elm) {
   ...
   if (_elm.CirclesModel.values) return _elm.CirclesModel.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var setRadius = function (size) {    var s = size / 30;return A3($Basics.clamp,5,80,$Basics.round(s));};
   var makeCircleSpec = function (_p0) {
      var _p1 = _p0;
      var _p7 = _p1._0;
      var radius = setRadius(_p1._1);
      var seed1 = $Random.initialSeed($Basics.round(_p7));
      var _p2 = A2($Random.generate,A2($Random.$int,10,50),seed1);
      var xv = _p2._0;
      var seed2 = _p2._1;
      var _p3 = A2($Random.generate,A2($Random.$int,10,50),seed2);
      var yv = _p3._0;
      var seed3 = _p3._1;
      var _p4 = A2($Random.generate,A2($Random.$int,10,220),seed3);
      var r = _p4._0;
      var seed4 = _p4._1;
      var _p5 = A2($Random.generate,A2($Random.$int,10,220),seed4);
      var g = _p5._0;
      var seed5 = _p5._1;
      var _p6 = A2($Random.generate,A2($Random.$int,10,220),seed5);
      var b = _p6._0;
      return {radius: radius,xv: xv,yv: yv,col: A3($Color.rgb,r,g,b),creationTime: _p7};
   };
   var Circle = F2(function (a,b) {    return {position: a,circleSpec: b};});
   var CircleSpec = F5(function (a,b,c,d,e) {    return {radius: a,xv: b,yv: c,col: d,creationTime: e};});
   var Position = F2(function (a,b) {    return {x: a,y: b};});
   return _elm.CirclesModel.values = {_op: _op,Position: Position,CircleSpec: CircleSpec,Circle: Circle,setRadius: setRadius,makeCircleSpec: makeCircleSpec};
};
Elm.CirclesView = Elm.CirclesView || {};
Elm.CirclesView.make = function (_elm) {
   "use strict";
   _elm.CirclesView = _elm.CirclesView || {};
   if (_elm.CirclesView.values) return _elm.CirclesView.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var drawCircle = F3(function (w,h,_p0) {
      var _p1 = _p0;
      var _p3 = _p1.position;
      var _p2 = _p1.circleSpec;
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: $Basics.toFloat(_p3.x) - $Basics.toFloat(w) / 2,_1: $Basics.toFloat(h) / 2 - $Basics.toFloat(_p3.y)},
      A2($Graphics$Collage.filled,_p2.col,$Graphics$Collage.circle($Basics.toFloat(_p2.radius))));
   });
   var drawCircles = F3(function (w,h,circles) {    return A3($Graphics$Collage.collage,w,h,A2($List.map,A2(drawCircle,w,h),circles));});
   var boundingBox = F2(function (w,h) {
      return A3($Graphics$Collage.collage,
      w,
      h,
      _U.list([A2($Graphics$Collage.outlined,$Graphics$Collage.solid($Color.black),A2($Graphics$Collage.rect,$Basics.toFloat(w),$Basics.toFloat(h)))
              ,A2($Graphics$Collage.outlined,$Graphics$Collage.solid($Color.black),A2($Graphics$Collage.rect,$Basics.toFloat(w - 2),$Basics.toFloat(h - 2)))]));
   });
   var view = F3(function (w,h,circles) {    return $Graphics$Element.layers(_U.list([A2(boundingBox,w,h),A3(drawCircles,w,h,circles)]));});
   var main = A3(view,
   400,
   400,
   _U.list([{circleSpec: {col: $Color.red,radius: 26},position: {x: 0,y: 0}},{circleSpec: {col: $Color.green,radius: 43},position: {x: 200,y: 200}}]));
   return _elm.CirclesView.values = {_op: _op,boundingBox: boundingBox,drawCircle: drawCircle,drawCircles: drawCircles,view: view,main: main};
};
...
Elm.Circles.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var computeCoordinate = F4(function (startingPointCoordinate,boxSize,velocity,time) {
      var distance = startingPointCoordinate + $Basics.round(velocity * time / 1000);
      var distanceMod = A2($Basics._op["%"],distance,boxSize);
      var distanceDiv = distance / boxSize | 0;
      return _U.eq(A2($Basics._op["%"],distanceDiv,2),0) ? distanceMod : boxSize - distanceMod;
   });
   var positionedCircle = F4(function (w,h,time,circle) {
      var _p0 = circle;
      var position = _p0.position;
      var circleSpec = _p0.circleSpec;
      var _p1 = circleSpec;
      var radius = _p1.radius;
      var xv = _p1.xv;
      var yv = _p1.yv;
      var creationTime = _p1.creationTime;
      var relativeTime = time - creationTime;
      var boxSizeX = w - radius * 2;
      var boxSizeY = h - radius * 2;
      var x = radius + A4(computeCoordinate,position.x - radius,boxSizeX,$Basics.toFloat(xv),relativeTime);
      var y = radius + A4(computeCoordinate,position.y - radius,boxSizeY,$Basics.toFloat(yv),relativeTime);
      return {position: {x: x,y: y},circleSpec: circleSpec};
   });
   var positionedCircles = F4(function (w,h,time,circles) {    return A2($List.map,A3(positionedCircle,w,h,time),circles);});
   var Down = {ctor: "Down"};
   var Up = function (a) {    return {ctor: "Up",_0: a};};
   var mousePressSignal = function () {
      var toMousePress = function (_p2) {    var _p3 = _p2;return _p3._1 ? Down : Up(_p3._0);};
      var acc = F2(function (_p5,_p4) {
         var _p6 = _p5;
         var _p9 = _p6._0;
         var _p8 = _p6._1;
         var _p7 = _p4;
         return _p8 ? {ctor: "_Tuple2",_0: _p9,_1: _p8} : {ctor: "_Tuple2",_0: _p9 - _p7._0,_1: _p8};
      });
      return A2($Signal$Extra._op["<~"],toMousePress,A3($Signal.foldp,acc,{ctor: "_Tuple2",_0: 0,_1: false},$Time.timestamp($Mouse.isDown)));
   }();
   var mouseUpSignal = A3($Signal.filter,
   function (press) {
      var _p10 = press;
      if (_p10.ctor === "Up") {
            return true;
   ...
   Up(0),
   mousePressSignal);
   var mouseDownTimeSignal = function () {
      var mouseDownTime = function (press) {    var _p11 = press;if (_p11.ctor === "Up") {    return _p11._0;} else {    return 0;}};
      return A2($Signal$Extra._op["<~"],mouseDownTime,mouseUpSignal);
   }();
   var clickPositionsSignal = A2($Signal.sampleOn,mouseUpSignal,$Mouse.position);
   var inBoxClickPositionsSignal = F2(function (w,h) {
      var positionInBox = function (pos) {    return _U.cmp($Basics.fst(pos),w) < 1 && _U.cmp($Basics.snd(pos),h) < 1;};
      return A3($Signal.filter,positionInBox,{ctor: "_Tuple2",_0: 0,_1: 0},clickPositionsSignal);
   });
   var creationTimeSignal = F2(function (w,h) {    return $Time.timestamp(A2($Signal.sampleOn,A2(inBoxClickPositionsSignal,w,h),mouseDownTimeSignal));});
   var newCircleSpecSignal = F2(function (w,h) {    return A2($Signal$Extra._op["<~"],$CirclesModel.makeCircleSpec,A2(creationTimeSignal,w,h));});
   var newCircleSignal = F2(function (w,h) {
      var makeCircle = F2(function (_p12,spec) {    var _p13 = _p12;return {position: {x: _p13._0,y: _p13._1},circleSpec: spec};});
      return A2($Signal$Extra._op["~"],A2($Signal$Extra._op["<~"],makeCircle,A2(inBoxClickPositionsSignal,w,h)),A2(newCircleSpecSignal,w,h));
   });
   var allCirclesSpecSignal = F2(function (w,h) {
      return A3($Signal.foldp,F2(function (x,y) {    return A2($List._op["::"],x,y);}),_U.list([]),A2(newCircleSignal,w,h));
   });
   var clockSignal = A2($Signal$Extra._op["<~"],$Basics.fst,$Time.timestamp($Time.fps(50)));
   var circlesSignal = F2(function (w,h) {
      return A2($Signal$Extra._op["~"],A2($Signal$Extra._op["<~"],A2(positionedCircles,w,h),clockSignal),A2(allCirclesSpecSignal,w,h));
   });
   var main = function () {
      var main$ = F2(function (w,h) {    return A2($Signal$Extra._op["<~"],A2($CirclesView.view,w,h),A2(circlesSignal,w,h));});
      return A2(main$,400,400);
   }();
   return _elm.Circles.values = {_op: _op
                                ,clockSignal: clockSignal
                                ,Up: Up
                                ,Down: Down
                                ,mousePressSignal: mousePressSignal
                                ,mouseUpSignal: mouseUpSignal
                                ,mouseDownTimeSignal: mouseDownTimeSignal
